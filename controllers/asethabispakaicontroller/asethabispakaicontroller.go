package asethabispakaicontroller
...
func Index(w http.ResponseWriter, r *http.Request) {
	...
	aset_tiks, err := asethabispakaimodel.GetAll(page, limit)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}

	totalRows, err := asethabispakaimodel.GetTotalRows()
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	helpers.RenderTemplate(w, "/aset/habis_pakai/index.html", data)
}
...
func Add(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		data := map[string]any{
			"Title":      "Add Aset TIK Habis Pakai",
			"path":       path,
			"categories": categories,
			"tipes":      tipes,
		}
		helpers.RenderTemplate(w, "/aset/habis_pakai/create.html", data)
	}
	...
	if r.Method == http.MethodPost {
		...
		// save the file
		path := "./public/uploads/aset/habispakai/"
		...
		dbPath := strings.ReplaceAll(filepath.Join("/public/uploads/aset/habispakai/", fileName), "\\", "/")
		...
		success, err := asethabispakaimodel.Create(aset_tik)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if success {
			http.Redirect(w, r, "/aset/habis-pakai", http.StatusSeeOther)
		} else {
			http.Error(w, "Failed to create aset", http.StatusInternalServerError)
		}
	}
}
...
func Edit(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		categories := categorymodel.GetAll()
		tipes := tipemodel.GetAll()
		aset_tik, err := asethabispakaimodel.Detail(idString)
		...
		helpers.RenderTemplate(w, "aset/habis_pakai/edit.html", data)
	}
	...
	if r.Method == http.MethodPost {
		...
		aset, err := asethabispakaimodel.Detail(idString)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
		}
		...
		success, err := asethabispakaimodel.Update(idString, aset_tik)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := asethabispakaimodel.Delete(idString); err != nil {
